...
def read_dat():
    ...
    datfile = '../secure/cf3_BGc_600_256/table3_linearDV.csv'
...
def interpgrid(l, b, cone=20, coordinates='supergalactic', veldist=-1, VD=10):
    ...
    Vx = np.load('../secure/cf3_BGc_600_256/VX_600_256.npy')
    Vy = np.load('../secure/cf3_BGc_600_256/VY_600_256.npy')
    Vz = np.load('../secure/cf3_BGc_600_256/VZ_600_256.npy')

    X = 600.*np.load('../secure/cf3_BGc_600_256/X_600_256.npy') / \
        h0/256.-600.*128/h0/256.
    Y = 600.*np.load('../secure/cf3_BGc_600_256/Y_600_256.npy') / \
        h0/256.-600.*128/h0/256.
    Z = 600.*np.load('../secure/cf3_BGc_600_256/Z_600_256.npy') / \
        h0/256.-600.*128/h0/256.
    ...
    if DDD:
        ...
        delta = 600./256./h0
        ii = np.floor((x0+300/h0)/delta)
        jj = np.floor((y0+300/h0)/delta)
        kk = np.floor((z0+300/h0)/delta)
    ...
    for i in range(ndis):
        ...
        delta = 600./256./h0
        ii = np.floor((x[i]+300/h0)/delta)
        jj = np.floor((y[i]+300/h0)/delta)
        kk = np.floor((z[i]+300/h0)/delta)
    ...
    for iii in ind:
        if dist_catalog[iii] < 300:
            ...
            delta = 600./256./h0
            ii = np.floor((x+300/h0)/delta)
            jj = np.floor((y+300/h0)/delta)
            kk = np.floor((z+300/h0)/delta)
    ...
    # ../secure/cf3_BGc_600_256/
    VgivesD = np.genfromtxt('../secure/cf3_BGc_600_256/VgivesD.csv',
