...
def read_dat():
    ...
    test = np.genfromtxt(datfile, delimiter=',',
                         filling_values=None, names=True, dtype=None, encoding=None)
...
def interpgrid(l, b, cone=20, coordinates='supergalactic', veldist=-1, VD=10):
    ...
    if (coordinates == 'blank'):
        ...
        p.line(np.arange(250), np.arange(250)*75, line_width=1,
               color="black", line_dash='dashed', legend_label='H0=75')
    ...
    DDD = False
    VVV = False
    if veldist == 0 and VD <= 200 and VD >= 0:
        DDD = True
    if veldist == 1:
        VVV = True
    ...
    # Density field in super galactic coordinates on grid 128x128x128 and box of 800 Mpc/h_75
    delta = f["arr_0"]
    # velocity field in super galactic coordinates on grid 128x128x128 and box of 800 Mpc/h_75
    v = f["arr_1"]
    ...
    curve = p.line('D_input', 'Vls', source=source, line_width=2,
                   color="blue", legend_label='Model')
...
# """))
    ...
    VgivesD = np.genfromtxt('../secure/cf3_model_romain/VgivesD.csv',
    ...
    p.line(Hub_d, Hub_Vls, line_width=1.5, color="red",
           line_dash='dashed', alpha=0.8, legend_label='H0=75')
...
# 3
    ...
    curve = p2.line('D_input', 'Vlsm', source=source,
                    line_width=2, color="blue", legend_label='Model')
    ...
    p2.line(np.arange(250), np.arange(250)*h0*100., line_width=1.5,
            color="red", line_dash='dashed', legend_label='H0=75')
# 3
